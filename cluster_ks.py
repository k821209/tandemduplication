...
import glob,subprocess, sys, kang, numpy
...
def convert_axt(filename):
	file_list = glob.glob('%s/%s.nuc.2.fas'%(outdir,filename)) # plank result in fas, second result
...
def get_Ks_list(seq1name,seq2name,seq1,seq2):
	...
	convert_axt('temp.fa.plankout')

	subprocess.call('/data/program/KaKs_Calculator1.2/src/KaKs_Calculator -i %s/temp.fa.plankout.result_of_2.axt -o %s/%s.%s.result_of_2.axt.LWL -m LWL '\
